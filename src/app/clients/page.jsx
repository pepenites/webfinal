...
export default function ClientsPage() {
  ...
  const handleEditClient = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Error en la respuesta API al editar el cliente');

      const updatedClient = await response.json();
      setClients(clients.map(client => (client._id === updatedClient._id ? updatedClient : client)));  // Actualizamos el cliente en la lista con el nuevo _id
  ...
  return (
    <div className="container">
      ...
      <ul className="clients-list">
        {clients.map(client => (
          <li key={client._id}> {/* Usamos _id en lugar de id */}
            <strong>ID:</strong> {client._id}  {/* Mostrar el _id del cliente aquí */}
            <p><strong>Nombre:</strong> {client.name}</p>
            <p>CIF: {client.cif}</p>
            <button onClick={() => setDetailsClient(client)} className="button secondary">
              Ver Detalles
            </button>
            <button onClick={() => setEditingClient(client)} className="button primary">
              Editar Cliente
            </button>
            <button onClick={() => handleDeleteClient(client._id)} className="button danger">
              Eliminar Cliente
            </button>
          </li>
        ))}
      ...
      {detailsClient && (
        <div className="client-details">
          ...
          <p><strong>ID:</strong> {detailsClient._id}</p>
          <p><strong>Nombre:</strong> {detailsClient.name}</p>
          <p><strong>CIF:</strong> {detailsClient.cif}</p>
          <p><strong>Calle:</strong> {detailsClient.address.street}</p>
          <p><strong>Número:</strong> {detailsClient.address.number}</p>
          <p><strong>Código Postal:</strong> {detailsClient.address.postal}</p>
          <p><strong>Ciudad:</strong> {detailsClient.address.city}</p>
          <p><strong>Provincia:</strong> {detailsClient.address.province}</p>
          <button onClick={() => setDetailsClient(null)} className="button secondary">
            Cerrar Detalles
          </button>
        </div>
      )}
